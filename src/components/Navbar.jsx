...
const MegaMenuPanel = ({ type, isOpen }) => {
  ...
  return (
    <div className="absolute top-full left-0 w-full min-h-[300px] border-t border-navy/10 shadow-2xl backdrop-blur-xl bg-white/95 z-50 transform origin-top transition-all duration-300">
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-10 flex">
        
        {/* Title Side */}
        <div className="w-1/3 pr-8 border-r border-navy/10">
          <h2 className="font-sans font-[700] text-navy text-3xl mb-4">{type}</h2>
        ...
        <div className="w-2/3 pl-12 grid grid-cols-2 gap-x-8 gap-y-6">
          {activeContent.map((item, idx) => (
            <Link to={`/${type.toLowerCase()}/${item.title.toLowerCase().replace(/ /g, '-')}`} key={idx} className="group cursor-pointer block outline-none">
              <h3 className="font-sans font-[600] text-navy text-[16px] mb-1 group-hover:text-electric transition-colors">{item.title}</h3>
              <p className="font-sans text-steel text-[13px] leading-[1.5] group-hover:text-navy transition-colors">{item.desc}</p>
            </Link>
          ))}
        </div>
  ...
  return (
    <nav 
      className={`fixed top-0 w-full z-50 transition-all duration-500 ${
        scrolled 
          ? 'py-3 lg:py-4 bg-white/90 backdrop-blur-xl border-b border-navy/10 shadow-[0_10px_30px_rgba(1,33,105,0.05)]' 
    ...
    >
      ...
      <div className={`fixed inset-0 bg-offwhite z-40 transform transition-transform duration-500 ease-in-out md:hidden ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full pt-20 px-6">
          <div className="flex justify-end mb-8">
            <button onClick={() => setMobileMenuOpen(false)} className="text-navy absolute top-6 right-6 p-2"><X size={28} /></button>
          </div>
          
          <div className="flex flex-col flex-grow overflow-y-auto pb-20">
            <Link to="/" className="mb-10 block" onClick={() => setMobileMenuOpen(false)}>
              <img src="/klhitech-logo.png" alt="KL Hi-Tech Logo" className="h-[36px] w-auto object-contain opacity-90" />
            </Link>
              ...
              {navItems.map((item, idx) => (
                <div key={idx} className="flex flex-col">
                  <div className="flex items-center justify-between py-2 border-b border-navy/5">
                    {item.to ? (
                      <Link to={item.to} className="text-xl font-medium text-navy hover:text-electric transition-colors" onClick={() => setMobileMenuOpen(false)}>
                        {item.label}
                      </Link>
                    ) : (
                      <span className="text-xl font-medium text-navy" onClick={() => item.hasDropdown && setActiveMenu(activeMenu === item.label ? null : item.label)}>
                        {item.label}
                      </span>
                    )}
                    {item.hasDropdown && (
                  ...
                  {item.hasDropdown && activeMenu === item.label && (
                    <div className="mt-4 flex flex-col space-y-4 pl-4 border-l border-navy/10">
                      {['Banking Solutions', 'Telecom Cards', 'Commercial Print', 'Digital Identities'].map((sub, sIdx) => (
                        <Link to={`/${item.label.toLowerCase()}/${sub.toLowerCase().replace(/ /g, '-')}`} onClick={() => setMobileMenuOpen(false)} key={sIdx} className="text-steel hover:text-navy transition-colors text-lg">{sub}</Link>
                      ))}
                    </div>
                  )}
                </div>
